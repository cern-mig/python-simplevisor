...
B<command> check, configuration_check, help, pod, restart, rst, single, start, status, stop, stop_children, stop_supervisor
...
B<--interval INTERVAL> interval to wait between supervision cycles

B<--log LOG> available: null, print, syslog, simple

B<--logfile LOGFILE> log file, ONLY for simple

B<--loglevel LOGLEVEL> log level, ONLY for simple and print

B<-p, --pidfile PIDFILE> the pidfile

B<--store STORE> file where to store the state
